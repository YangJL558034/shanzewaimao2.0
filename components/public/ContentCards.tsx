...
export function CategoryCard({ category, index, viewLabel = "View More" }: { category: CategoryCardData; index: number; viewLabel?: string }) {
  return <article className="card category-card"><div className="card-art" style={{ "--art-image": `url(${category.image || "/references/products.png"})`, "--art-position": category.image ? "center" : positions[index%positions.length] } as React.CSSProperties}/><div className="card-body"><h3>{category.name}</h3><p>{category.description}</p><Link className="text-link" href={`/products?category=${category.slug}`}>{viewLabel} →</Link></div></article>;
}
...
function PrimaryProductMedia({ source, position }: { source: string | null; position: string }) {
  ...
  return <div className="card-art product-card-primary-media" style={{ "--art-image": `url(${source || "/references/products.png"})`, "--art-position": source ? "center" : position } as React.CSSProperties} />;
}
  ...
  return <div className="grid-cards product-grid">{products.map((product, i) => {
    ...
    return <article className={`card product-card ${product.hoverImage ? "has-hover-image" : ""}`} key={product.id}>
      ...
      <div className="product-card-media" aria-label={product.name}>
        ...
        {product.hoverImage && <div className="card-art product-card-hover-media" style={{ "--art-image": `url(${product.hoverImage})`, "--art-position": "center" } as React.CSSProperties} />}
      </div>
