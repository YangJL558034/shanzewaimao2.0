...
export const cmsSections: CmsSection[] = [
  { key: "products", label: "产品管理", singular: "产品", model: "product", permission: "products", description: "产品资料、规格、特性、应用和发布状态", listFields: ["name", "sku", "status", "isFeatured", "updatedAt"], fields: [
    ...
    { key: "coverImage", label: "① 产品正面主图或视频（默认显示）", type: "visualMedia", help: "产品列表默认显示的正面媒体；可点击“从本地上传”或“从媒体库选择”。" }, { key: "hoverImage", label: "② 产品反面图片（鼠标悬停显示）", type: "media", help: "请点击本栏自己的“从本地上传”按钮上传反面图。桌面端鼠标放到产品卡片时会平滑切换，移动端仍显示正面图。" }, { key: "galleryImages", label: "③ 产品详情图片与视频轮播", type: "visualGallery", virtual: true, help: "可混合添加多张图片、本地视频和视频平台链接，并调整播放顺序" }, { key: "specifications", label: "产品规格", type: "json", jsonKind: "keyValue", required: true, translatable: true },
  ...
  { key: "categories", label: "产品分类", singular: "分类", model: "productCategory", permission: "products", description: "前台产品分类与排序", listFields: ["name", "slug", "sortOrder", "isVisible"], fields: [
    ...
    { key: "image", label: "分类图片", type: "media" }, { key: "icon", label: "图标名称" }, { key: "sortOrder", label: "排序", type: "number" }, { key: "isVisible", label: "前台显示", type: "boolean" },
  ]},
  ...
  { key: "banners", label: "首屏与轮播管理", singular: "轮播图", model: "banner", permission: "pages", description: "管理各页面最上方的大标题、说明、按钮和背景图；首页多条记录组成轮播", listFields: ["title", "page", "status", "sortOrder"], fields: [
      { key: "key", label: "系统识别编号", required: true, help: "系统用于识别内容，通常无需修改" }, { key: "page", label: "显示在哪个页面", type: "select", required: true, options: pageOptions }, { key: "title", label: "主标题", required: true, translatable: true }, { key: "subtitle", label: "蓝色强调标题", translatable: true }, { key: "description", label: "标题下方说明", type: "textarea", translatable: true }, { key: "image", label: "首屏背景图片或视频", type: "visualMedia", help: "可本地上传图片、MP4/WebM 视频，或粘贴 YouTube、Vimeo、Bilibili、Youku 视频链接。多条记录每 2 秒切换。" }, { key: "ctaLabel", label: "按钮显示文字", translatable: true }, { key: "ctaHref", label: "按钮点击后跳转地址" }, { key: "sortOrder", label: "轮播顺序", type: "number" }, { key: "status", label: "前台显示状态", type: "select", options: statusOptions },
  ]},
