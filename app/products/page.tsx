...
import { CategoryCards, ProductCards } from "@/components/public/ContentCards";
...
export default async function ProductsPage({ searchParams }: { searchParams: Promise<{ category?: string }> }) {
  ...
  const [categories, products, rawBanners, section] = await Promise.all([
    ...
    db.product.findMany({ where: { status: "PUBLISHED", ...(category ? { category: { slug: category } } : {}) }, orderBy: [{ isFeatured: "desc" }, { sortOrder: "asc" }] }),
  ...
  return <>
    ...
    {categoriesSection && <section className="section"><div className="container-site"><div className="center-head"><div className="eyebrow">{categoriesSection.subtitle}</div><h2 className="section-title">{categoriesSection.title}</h2><p>{categoriesSection.content}</p></div><CategoryCards categories={localizedCategories} viewLabel={categoriesSection.buttonLabel || (zh ? "查看更多" : "View More")} /></div></section>}
    {featured && <section className="section section-soft"><div className="container-site"><div className="center-head"><div className="eyebrow">{featured.subtitle}</div><h2 className="section-title">{category ? `${localizedCategories.find((item) => item.slug === category)?.name || (zh ? "分类" : "Category")} ${zh ? "产品" : "Products"}` : featured.title}</h2><p>{featured.content}</p></div><ProductCards products={localizedProducts} detailLabel={featured.buttonLabel || (zh ? "查看详情" : "View Details")} newLabel={zh ? "新品" : "NEW"} /></div></section>}
